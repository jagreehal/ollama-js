...
| Fork compatibility patch                             | Extends per-request cancellation to embed and web methods | Gives every client method used by `ai-sdk-ollama` the same contract |
...
3. Link every fork-only change to an upstream issue, pull request, or an
   explicit `ai-sdk-ollama` compatibility test.
